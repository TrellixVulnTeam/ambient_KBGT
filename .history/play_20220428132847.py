            ...
            for j in range(len(current_time_frame)):
                ...
                for m in range(len(note_time)):
                    ...
                    for n in range(len(phrase_chord)):
                        midiout.send_message(musx.note_on(n+1, phrase_chord[0], vel))
                    midiout.send_message(musx.note_on(2, phrase_chord[1], vel))
                    midiout.send_message(musx.note_on(3, phrase_chord[2], vel))
                    midiout.send_message(musx.note_on(4, phrase_chord[3], vel))
                    print(f'chord {j+1}, note: {phrase_chord[0], phrase_chord[1], phrase_chord[2], phrase_chord[3]}, \n')
                    time.sleep(note_time[m])
                    if m == len(note_time)-1:
                        midiout.send_message(musx.note_off(1, phrase_chord[0], vel))
                        midiout.send_message(musx.note_off(2, phrase_chord[1], vel))
                        midiout.send_message(musx.note_off(3, phrase_chord[2], vel))
                        midiout.send_message(musx.note_off(4, phrase_chord[3], vel))
